...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx, search: str, *, loop, download=False):
        ...
        return cls(discord.FFmpegPCMAudio(source, executable="C:/FFmpeg/bin/ffmpeg.exe"), data=data,
...
class Music(commands.Cog):
    ...
    async def resume_(self, ctx):
        ...
        vc.resume()
        await ctx.message.add_reaction("\U000023ef")
    ...
    async def remove_(self, ctx, pos: int = None):
        ...
        if pos == None:
            ...
            try:
                ...
                embed = discord.Embed(title="", description=f"Couldn't find a track for {pos}",
                                      color=discord.Color.red())
                await ctx.send(embed=embed)
    ...
    async def queue_info(self, ctx):
        ...
        fmt = '\n'.join(
            ...
            f"{_['requester']}`\n"
            for _ in upcoming)
        fmt = f"\n__Now Playing__:\n[{vc.source.title}]({vc.source.web_url}) | ` {duration} Requested by: " \
              f"{vc.source.requester}`\n\n__Up Next:__\n" + fmt + f"\n**{len(upcoming)} songs in queue**"
        embed = discord.Embed(title=f'Queue for {ctx.guild.name}', description=fmt, color=discord.Color.from_rgb(0, 255, 255))
    ...
    async def now_playing_(self, ctx):
        ...
        embed = discord.Embed(title="",
                              description=f"[{vc.source.title}]({vc.source.web_url}) [{vc.source.requester.mention}] | "
                                          f"`{duration}`",
                              color=discord.Color.from_rgb(0, 255, 255))
